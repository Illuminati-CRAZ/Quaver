...
{
	public class MapPreviewContainer : Sprite
	{
		/// <summary>
		/// </summary>
		...
		/// <summary>
		/// </summary>
		private TaskHandler<Qua, GameplayScreen> LoadGameplayScreenTask { get; }
		...
		/// <summary>
		/// </summary>
		protected Bindable<Qua> Qua { get; set; }

		/// <summary>
		/// 	A deep copy of Qua to apply mods on for the preview
		/// </summary>
		protected Qua PreviewQua { get; set; }
		...
		/// <summary>
		/// </summary>
		public MapPreviewContainer(Bindable<bool> isPlayTesting, Bindable<LeftPanel> activeLeftPanel, int height,
			IAudioTrack track = null, Bindable<Qua> qua = null)
		{
			IsPlayTesting = isPlayTesting;
			ActiveLeftPanel = activeLeftPanel;
			Qua = qua ?? new Bindable<Qua>(null){ Value = null };
			Track = track;
			Size = new ScalableVector2(564, height);
			Alpha = 0f;

			LoadGameplayScreenTask = new TaskHandler<Qua, GameplayScreen>(LoadGameplayScreen);
		...
		/// <returns></returns>
		private GameplayScreen LoadGameplayScreen(Qua map, CancellationToken token)
		{
			...
			return map is null ? null : HandleLoadGameplayScreen(map, token);
		}
		...
		/// <param name="token"></param>
		/// <returns></returns>
		private GameplayScreen HandleLoadGameplayScreen(Qua map, CancellationToken token)
		{
			try
			{
				PreviewQua = ObjectHelper.DeepClone(map);

				PreviewQua.ApplyMods(ModManager.Mods);

				var autoplay = Replay.GeneratePerfectReplayKeys(new Replay(PreviewQua.Mode, "Autoplay", 0, ""), PreviewQua);

				var gameplay = new GameplayScreen(PreviewQua, "", new List<Score>(), autoplay, true, 0,
		...
		/// <param name="e"></param>
		private void OnLoadedGameplayScreen(object sender, TaskCompleteEventArgs<Qua, GameplayScreen> e)
		{
			if (e.Result == null)
				return;
			...
			AddScheduledUpdate(() =>
			{
				...
				ShowTestPlayPrompt();
				CreateSeekBar(PreviewQua, playfield);
			});
		}
		...
		/// <param name="sender"></param>
		/// <param name="e"></param>
		private void OnMapChanged(object sender, BindableValueChangedEventArgs<Qua> e)
		{
			if (LoadedGameplayScreen != null)
			{
				if (e.OldValue != null)
		...
		protected void RunLoadTask()
		{
			...
			LoadGameplayScreenTask.Run(Qua.Value, DelayTime);
		}
        ...
        /// <summary>
        /// </summary>
        private void CreateTestPlayPrompt() => TestPlayPrompt = new SpriteTextPlus(FontManager.GetWobbleFont(Fonts.LatoBlack),
